...
class AnniversaryPage {
  ...
  animateTimelineItem(item) {
    ...
    } // Trigger bubble burst at timeline item position
  ...
  showTimelineDetail(item) {
    const title =
      item.querySelector(".timeline-title, .milestone-title")?.textContent ||
      "Memory";
    const date =
      item.querySelector(".timeline-date, .milestone-date")?.textContent || "";
    const description =
      item.querySelector(".timeline-description, .milestone-description")
        ?.textContent || "";
   ...
   */
  initializeMemories() {
    const memoriesContainer = document.querySelector(
      ".memories, .photo-memories, #memories",
    );
    if (!memoriesContainer) return;

    const photos = memoriesContainer.querySelectorAll(
      ".memory-photo, .photo-item",
    );
  ...
  initializeCelebrationEffects() {
    ...
    const celebrationTriggers = document.querySelectorAll(
      ".celebrate-btn, .celebration-trigger",
    );
  ...
  triggerCelebrationEffect() {
    // Bubble burst at center
    const centerX = window.innerWidth / 2;
    const centerY = window.innerHeight / 2;

    this.triggerBubbleBurst(centerX, centerY);

    // Multiple bursts
    setTimeout(() => this.triggerBubbleBurst(centerX - 100, centerY - 50), 200);
    setTimeout(() => this.triggerBubbleBurst(centerX + 100, centerY - 50), 400);
    setTimeout(() => this.triggerBubbleBurst(centerX, centerY + 100), 600);
  ...
  setupInteractions() {
    ...
    loveButtons.forEach((button) => {
      button.addEventListener("click", (e) => {
        this.triggerBubbleBurst(e.clientX, e.clientY);
      });
    });
  }
