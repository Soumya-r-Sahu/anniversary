...
class UnifiedGallery {
  ...
  updateArrows() {
    if (this.elements.prevBtn) {
      this.elements.prevBtn.style.opacity =
        this.state.totalSlides > 1 ? "1" : "0.5";
    }
  ...
  onTouchEnd(event) {
    ...
    if (deltaTime <= this.touch.allowedTime) {
      if (
        Math.abs(deltaX) >= this.touch.threshold &&
        Math.abs(deltaY) <= this.touch.restraint
      ) {
        if (deltaX < 0) {
