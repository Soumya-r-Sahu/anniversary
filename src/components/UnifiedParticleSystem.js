...
class UnifiedParticleSystem {
    ...
    setupCanvas() {
        ...
        this.ctx = this.canvas.getContext('2d', {
            alpha: true,
            desynchronized: true,
            powerPreference: this.performance.isLowPower ? 'low-power' : 'high-performance'
        });
    ...
    setupEventListeners() {
        ...
        if (this.config.enableInteraction && this.canvas) {
            this.canvas.addEventListener('mousemove', this.boundHandlers.onMouseMove, { passive: true });
            this.canvas.addEventListener('mouseenter', this.boundHandlers.onMouseEnter, { passive: true });
            this.canvas.addEventListener('mouseleave', this.boundHandlers.onMouseLeave, { passive: true });
            this.canvas.addEventListener('click', this.boundHandlers.onClick, { passive: true });
        }
    }
    ...
    animate() {
        ...
        if (deltaTime >= this.performance.frameInterval) {
            if (!this.state.isPaused && this.state.isVisible && this.performance.shouldRender) {
                this.update();
                this.render();
            }
    ...
    destroy() {
        ...
        if (this.canvas) {
            ...
            this.canvas.removeEventListener('mouseenter', this.boundHandlers.onMouseEnter);
...
window.particlesJS = function(containerId, config) {
    ...
    const unifiedConfig = {
        ...
        particleColor: config.particles?.color?.value || ["#ff6b9d", "#ffa8cc", "#ffb3d6"],
        particleSize: {
            min: config.particles?.size?.value ? config.particles.size.value * 0.5 : 1,
            max: config.particles?.size?.value || 4
        },
        particleSpeed: {
            min: config.particles?.move?.speed ? config.particles.move.speed * 0.5 : 0.5,
