...
class LazyLoader {
  ...
  async loadImage(img) {
    ...
    try {
      ...
      // Fire custom event
      img.dispatchEvent(
        new CustomEvent("lazy-loaded", { detail: { src: optimizedSrc } }),
      );
    } catch (error) {
  ...
  async loadContent(el) {
    ...
    try {
      ...
      el.dispatchEvent(
        new CustomEvent("content-loaded", { detail: { url: contentURL } }),
      );
    } catch (error) {
  ...
  getStats() {
    return {
      ...
      pendingImages: document.querySelectorAll("[data-src], [data-bg-src]")
        .length,
